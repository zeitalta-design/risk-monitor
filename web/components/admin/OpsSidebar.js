...
/**
 * Phase228: 運営管理画面サイドバーナビ
 * - 青基調 / 極太ゴシック / 強いUI
 * - 運営の司令塔として一目で現在地がわかる設計
 */
...
export default function OpsSidebar() {
  ...
  useEffect(() => {
    fetch("/api/auth")
      .then((res) => res.json())
      .then((data) => {
        if (data.user?.role === "admin") setAdminUser(data.user);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <aside className="w-60 bg-gray-900 min-h-screen flex flex-col shrink-0 print:hidden">
      {/* ロゴ */}
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        {NAV_SECTIONS.map((section) => (
          <div key={section.label} className="mb-6">
            <h3 className="px-5 mb-2 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider">
              {section.label}
            </h3>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className={`flex items-center gap-3 px-5 py-2.5 text-sm font-bold transition-colors ${
                        active
...
function InquiryIcon({ className }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
function ScrapingIcon({ className }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
